...
| # | 项 | 核验方式 | 结论（实测） |
|---|----|----------|--------------|
| P0-1 | 仓库无远程 | `git remote -v` 执行 | **已解决（2026-09-17）**——远程 `git@github.com:likui9969-alt/web-automation.git`，SSH 推送，仓库 public，39 个文件与本地逐位一致 |
| P0-2 | CI 从未真实运行 | 触发 GitHub Actions 实跑 | **已解决（2026-09-17）**——run `35199658118`（commit `100e3f2`）**全绿 / 1m59s**，各步骤 success、Allure 结果已上传。前 3 次失败暴露并修掉 2 个真实缺陷（`up` 未限定服务 / 容器内写不了 reports 导致 pytest INTERNALERROR），另补 CI 缺失的 DROP 步骤。**"CI 已跑通"现在可以写** |
| P0-3 | README 状态节停在 M6 | 读取 README.md 状态节 | **已解决**——逐模块更新至 M11，含「状态说明」诚实声明（M9 已跑通 / M10 独立复审为开放项） |
