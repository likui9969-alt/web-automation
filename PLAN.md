...
### M0 需求分析与测试设计（已按「M0-lite」收口，2026-09-15 所有者决策）

- **为什么**：测试开发的第一步是分析被测系统，不是写脚本。
- **收口决策（全面复审 P1-1，二选一中选 B）**：原计划的三模块测试点清单（PIM/Leave/Recruitment 各 ≥8 条）未在 M0 交付，**显式降级为 M0-lite（登录模块试点）**，取舍与去向：
  - **已完成**：被测系统摸底、四条硬约束（C1–C4）、登录模块 15 条测试点实测、测试金字塔、架构草案、本计划。
  - **PIM 测试点**：不再补前置清单——已由 M4–M6 实践路径实质覆盖（抓包探测 → API 用例 → e2e/db 闭环用例，均有实测记录）。取舍理由：整模块预设计在共享环境上验证成本过高，按需设计 + 实证沉淀更符合本项目节奏。
  - **Leave/Recruitment 测试点**：作为对应模块的**开工前置条件**（本节即为排期承诺）。
  - **思考题 ×3**：不做独立文档——答案已沉淀在各模块实现中（全 UI 化弊端 → 金字塔 3 倍速差实测；共享环境数据 → C2 双实证；UI vs API 断言差异 → e2e 双向闭环设计），M11 复盘统一整理。
- **交付物**：PLAN.md、登录模块作业（实测 15 条测试点）、本收口决策记录。
- **DoD**：登录模块试点已评审（REVIEW_FEEDBACK M1 节）→ 用户能讲清分层理由（M1–M6 各 Review 面试能力评估持续覆盖）。
...
> 进度唯一事实源：[MODULE_FEEDBACK.md](MODULE_FEEDBACK.md) 状态表。
> 本节曾维护进度快照导致两处漂移（全面复审 P2-3），现改为只指向事实源，不再快照。
